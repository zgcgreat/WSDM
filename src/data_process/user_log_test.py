...
def get_unique_users():
    df_train_v2 = pd.read_csv(in_path + 'sample_submission.csv', usecols=['msno'])
    # df_test = pd.read_csv(in_path + 'sample_submission.csv', usecols=['msno'])
    # df = pd.concat([df_train_v2, df_test], axis=0)
    unique_users = set(df_train_v2['msno'].unique())
    df_train = []
    df_test = []
    df = []
...
def drop_user_log(unique_users):
    fo = open(cache_path + 'user_log_test.csv', 'w')  # train_v2, test
...
def get_user_log():
    chunksize = size
    last_user_logs = pd.DataFrame()
    df_iter = pd.read_csv(cache_path + 'user_log_test.csv', iterator=True, chunksize=chunksize, dtype={'num_25': np.int8,
                                                                                              'num_50': np.int8,
    ...
    for i, df in enumerate(df_iter, start=1):
        ...
        del df_stats
        gc.collect()
        last_user_logs = last_user_logs.append(df)
        df = []
        del df
        gc.collect()
        print(i * chunksize)
    ...
    # last_user_logs.to_csv(out_path + 'tmp.csv', index=False)
    last_user_logs = transform_df(last_user_logs)
    last_user_logs = parallel(last_user_logs, transform_date)
    last_user_logs = last_user_logs.drop(['date'], axis=1)
    last_user_logs.to_csv(out_path + 'user_log_test.csv', index=False, chunksize=chunksize)


def func():
    chunksize = size
    df_all = pd.DataFrame()
    df_iter = pd.read_csv(out_path + 'tmp.csv', iterator=True, chunksize=chunksize,
    ...
    df_all.to_csv(out_path + 'user_log_test.csv', index=False)
