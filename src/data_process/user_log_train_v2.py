...
def drop_user_log(unique_users):
    fo = open(cache_path + 'user_log_train_v2.csv', 'w')  # train_v2, test
...
def get_user_log():
    ...
    df_iter = pd.read_csv(cache_path + 'user_log_train_v2.csv', iterator=True, chunksize=chunksize, dtype={'num_25': np.int8,
                                                                                              'num_50': np.int8,
    ...
    last_user_logs.to_csv(cache_path + 'tmp.csv', index=False)
...
def func():
    ...
    df_all.to_csv(out_path + 'user_log_train_v2.csv', index=False)
