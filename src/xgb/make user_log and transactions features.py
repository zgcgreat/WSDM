...
def make_transactions_features():
    print('loading...')
    infos = {}
    with open(in_path + 'transactions.csv') as fd:
        count = 0
        fd.readline()
        for line in fd:
            pos = line.find(',')
...
def make_userlog_features():
    print('loading...')
    infos = {}
    with open(in_path + 'user_logs.csv') as fd:
        count = 0
        fd.readline()
        for line in fd:
            pos = line.find(',')
            msid = line[:pos]
            # print(pos, msid)
            # num_25, num_50, num_75, num_985, num_100, num_unq, total_secs = [int(float(value)) for value in line[pos + 1:-1].split(',')]
            splits = line[pos + 1:-1].split(',')
...
df_members = pd.read_csv(in_path + 'members.csv')
...
df_members['registration_init_date'] = df_members['registration_init_time'].apply(lambda x: int(str(x)[-2:])).astype(
        np.int8)
df_members['expiration_date_year'] = df_members['expiration_date'].apply(lambda x: int(str(x)[:4])).astype(np.int16)
df_members['expiration_date_month'] = df_members['expiration_date'].apply(lambda x: int(str(x)[4:6])).astype(np.int8)
df_members['expiration_date_date'] = df_members['expiration_date'].apply(lambda x: int(str(x)[-2:])).astype(np.int8)
df_members = df_members.drop('registration_init_time', 1)
df_members = df_members.drop('expiration_date', 1)

train = pd.merge(train, df_members, how='left', on='msno')
test = pd.merge(test, df_members, how='left', on='msno')
...
test[['msno', 'is_churn']].to_csv(output_path + 'xgb_submission{}.csv'.format(datetime.now().strftime("%Y%m%d-%H%M%S")), index=False)
