...
def stacking(clf, train_x, train_y, test_x, clf_name, class_num=2):
    ...
    for i, (train_index, test_index) in enumerate(kf):
        ...
        if clf_name in ["rf", "ada", "gb", "et", "lr", "knn", "mnb", "ovr", "gnb"]:
            ...
            # clf.add(SReLU())
            # clf.add(BatchNormalization())
            clf.add(Dense(256, activation="relu", W_regularizer=l2()))
...
def get_data():
    ...
    drop_list = ['registration_init_time', 'msno_hash', 'total_order',
                 'transaction_date']  # 'registration_init_time', 'msno_hash', 'total_order', 'transaction_date'
...
if __name__ == "__main__":
    ...
    # x_train, x_test = gen_feature(x_train, x_test)
    ...
    # train.to_csv("../data/train_stacking.csv", index=None)
    # test.to_csv("../data/test_stacking.csv", index=None)
    ...
    #           index=False)
    print(log_loss(y_test, df['is_churn']))
