...
FIELD_SIZES = [0] * 26
with open('../../output/fm/featindex.txt') as fin:
    for line in fin:
        line = line.strip().split(':')
        if len(line) > 1:
            f = int(line[0])-1  # f = int(line[0])-1
            FIELD_SIZES[f] += 1
print('field sizes:', FIELD_SIZES)
FIELD_OFFSETS = [sum(FIELD_SIZES[:i]) for i in range(len(FIELD_SIZES))]
INPUT_DIM = sum(FIELD_SIZES)  # 特征取值个数
OUTPUT_DIM = 1
STDDEV = 1e-3
MINVAL = -1e-3
MAXVAL = 1e-3
...
def read_data(file_name):
    ...
    X = libsvm_2_coo(zip(X, D), (len(X), INPUT_DIM)).tocsr()  # zip(X, D) 将X,D合并
    return X, y
...
def libsvm_2_coo(libsvm_data, shape):
    ...
    return coo_matrix((coo_data, (coo_rows, coo_cols)), shape=shape)  # 稀疏矩阵
...
def slice(csr_data, start=0, size=-1):
    if not isinstance(csr_data[0], list):  # 判断变量类型是否为list
